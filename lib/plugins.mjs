...
export async function install(source, id) {
  ...
  if (existsSync(manifestPath)) {
    ...
    const entryFile = entry.entry || pkg.main || 'server.mjs';
    manifest = {
      id,
...
/** Execute an active ping or verification for a plugin instance */
export async function ping(id) {
  const dir = pluginDir(id);
  if (!existsSync(dir)) return { ok: false, error: 'Plugin directory not found' };

  const pingScript = join(dir, 'ping.mjs');
  if (existsSync(pingScript)) {
    try {
      const out = await exec('node', ['ping.mjs'], { cwd: dir, timeout: 15000 });
      try {
        const parsed = JSON.parse(out.trim());
        return { ok: true, detail: parsed.detail || 'Ping succeeded' };
      } catch {
        return { ok: true, detail: out.trim() || 'Ping succeeded' };
      }
    } catch (err) {
      let msg = err.message;
      try {
        const parsed = JSON.parse(err.message.trim());
  ...
  if (st.state === 'connected') {
    return { ok: true, detail: 'Configuration present' };
  } else {
    return { ok: false, error: st.detail || 'Needs configuration' };
  }
}
