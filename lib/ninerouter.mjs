...
export function getQuotaPolicyState() {
  try {
    ...
    try {
      const settingsRaw = readFileSync(join(DATA, 'quota-policy-settings.json'), 'utf8');
      state.settings = JSON.parse(settingsRaw);
    } catch {
      state.settings = { averagedUsage: true };
    }
    return state;
  } catch {
    return null;
...
/** Update quota policy settings */
export function setQuotaPolicySettings(settings) {
  const current = getQuotaPolicyState()?.settings || { averagedUsage: true };
  const updated = { ...current, ...settings };
  writeFileSync(join(DATA, 'quota-policy-settings.json'), JSON.stringify(updated, null, 2) + '\n');
  return updated;
}
